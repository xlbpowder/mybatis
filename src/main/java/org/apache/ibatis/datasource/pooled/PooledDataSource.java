...
public class PooledDataSource implements DataSource {
    ...
    /**
     * 一个连接在 {@link PoolState#activeConnections} 中的最长时间（即最大activeTime），超过该时间会被新的请求连接的线程强制占用并且autoCommit为false的时候事务也会被回滚。单位：毫秒
     */
    ...
    /**
     * 期望 Connection 的连接信息编码，用来判断两个连接的信息是否相同或者连接池是否兼容某个连接，通过 {@link #assembleConnectionTypeCode(String, String, String)} 计算。
     */
        ...
        synchronized (state) {
            ...
            if (conn.isValid()) { // 有效
                ...
                if (state.idleConnections.size() < poolMaximumIdleConnections && conn.getConnectionTypeCode() == expectedConnectionTypeCode) {
                    ...
                    // 唤醒所有可能因为idleConnections为空、activeConnections又满了而正在等待连接的线程
                    state.notifyAll();
                } else {
                    // 统计连接使用时长
                    state.accumulatedCheckoutTime += conn.getCheckoutTime();
            ...
            synchronized (state) {
                ...
                // 无空闲空闲连接
                    ...
                    if (state.activeConnections.size() < poolMaximumActiveConnections) {
                        ...
                        // 获得最早激活的 PooledConnection 对象
                        PooledConnection oldestActiveConnection = state.activeConnections.get(0);
                        // 检查该连接是否超时（该连接active的时间是否超出最大时间）
                        ...
                        if (longestCheckoutTime > poolMaximumCheckoutTime) { // 检查到超时
                            ...
                            // 没有空闲的连接，activeConnections又满了，且最早的active连接还未超时，所以来到这里，能做的只有wait，等待有连接使用完被close()，然后push进idleConnections然后被notify（其实还可以由activeConnections中的连接超时来notify，可能因为相对复杂，这里只是简单的连接池实现）
                            // Must wait
                            try {
                ...
                if (conn != null) {
                    ...
                    if (conn.isValid()) {
                        ...
                        // 将 conn 置空，那么可以继续获取（bad connection无论是从idleConnections还是activeConnections中获取的抑或是直接新建的都已经被移除引用了，都会被GC掉）
                        conn = null;
    ...
    public static Connection unwrapConnection(Connection conn) {
        // 如果传入的是一个代理对象，则获取其代理处理类，然后获取真实的连接；否则直接返回conn
