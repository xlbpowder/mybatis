 ...
 *
 * <a href='https://blog.csdn.net/rainbow702/article/details/63255736'>BUG</a>
 *
 *
 * @author Ben Gunter
 */
public class DefaultVFS extends VFS {
    ...
    /** The magic header that indicates a JAR (ZIP) file. <p>JAR或者ZIP文件的头几个字节</p> */
    ...
    protected List<String> listResources(JarInputStream jar, String path) throws IOException {
        ...
        for (JarEntry entry; (entry = jar.getNextJarEntry()) != null; ) {
            if (!entry.isDirectory()) {
                ...
                // Check file name
                // TODO：搞不懂这里为什么要判断是path开头的，来得到这里的，path都是带有.jar结尾的，这样就要求所有jar中的有效文件的包都包括这个path才可以被作为子资源返回，
                //  这样就获取不到jar中的class了，那这个读取jar中资源的意义何在？？？？？？在尝试修改path的值为不含.jar的时候确认jar中class文件可以被返回了，但是不能被类
                //  加载器加载，可能是因为这个class不在类路径上，那java应该不是通过这种方式加载jar中的类的，那么本类对于jar的这个动作又意义何在呢？？？
     ...
     * <p>示例：org.apache.ibatis.io  ---->  org/apache/ibatis/io</p>
     *
    ...
    /**
     * Returns true if the resource located at the given URL is a JAR file.
     * <p>判断{@code url}指向的文件是否以{@link #JAR_MAGIC}这几个字节开头</p>
     *
     * @param url The URL of the resource to test.
