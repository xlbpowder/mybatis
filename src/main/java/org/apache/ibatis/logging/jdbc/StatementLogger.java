...
public final class StatementLogger extends BaseJdbcLogger implements InvocationHandler {
    ...
    /**
     * {@link Statement}对象没有setXxx方法，所以这里不会像{@link PreparedStatementLogger}那样打印参数值。它只有{@link #EXECUTE_METHODS}方法，并且第一个参数值都是有值的，为sql，
     * 所以这里都会打印sql（调用{@link #removeBreakingWhitespace(String)}）。如果方法是"executeQuery"和"getResultSet"，则会构建一个{@link ResultSetLogger}返回
     *
     * @param proxy
     * @param method
     * @param params
     ...
     * 构建{@link Statement}代理对象{@link StatementLogger}
     *
